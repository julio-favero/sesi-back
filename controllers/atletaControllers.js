...
class atletaControllers {
  ...
  static async listUsersOne(req, res) {
    ...
    let data = await atletaModels.findOne({ raw: true, where: { email: req.body.email }, attributes: ['nome', 'cargo', 'idatleta'] }); // Listar dados sobre ele mesmo
  ...
  // ----------------------- Visualizar Guia -----------------------
  static async visualizarGuia(req, res) {
    let arrayExame = []
    let arrayId = []
