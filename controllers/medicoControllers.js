...
class medicoControllers {
    ...
    static async listAtlheteSolicited(req, res) {
        try {
            ...
            const response = await examesModels.findAll({raw: true, where: {pdfatleta: {[Op.not]: null,},},});
      
            const idSet = new Set();
            const arrayObj = [];
        
            for (let i = 0; i < response.length; i++) {
                const id_exame_atl = response[i].id_exame_atl;
                if (!idSet.has(id_exame_atl)) {

                idSet.add(id_exame_atl);
        
                const atleta = await atletaModels.findOne({raw: true, where: { idatleta: id_exame_atl , id_medico_atl: idMedico},});
        
                if (atleta) {
                    arrayObj.push(atleta);
                }
                }
            }      
          ...
          res.status(500).json({ error: 'Erro ao buscar atletas solicitados.' });
        }
      }
    ...
    static async solicitarExame(req, res) {
        ...
        let pdf = fs.readFileSync(
          ...
          ); // Lê o conteúdo arquivo específico por esse caminho e salva na variável pdf
        ...
        try {
            ...
            if (x === 0) {
                ...
                const novoExame = await examesModels.create(req.body)
                const idExame = novoExame.idexame;
